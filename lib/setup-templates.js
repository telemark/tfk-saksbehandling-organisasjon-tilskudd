...
module.exports = miss.through((chunk, encoding, callback) => {
  ...
  documentIn.title = `Søknad tilskuddsmidler - ${midlerType} - ${escape(item.tiltak.navn)}`
  documentIn.offTitle = `Søknad tilskuddsmidler - ${midlerType} - ${escape(item.tiltak.navn)}`

  documentOut.title = `Bekreftelse på mottatt søknad - ${escape(item.tiltak.navn)}`
  documentOut.offTitle = `Bekreftelse på mottatt søknad - ${escape(item.tiltak.navn)}`
